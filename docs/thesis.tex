...
I2P splits the information about a peer's anonymous identity and its real identity by having the peer add its lease information to the database via an outbound tunnel. Information about the services that the peer hosts or uses are then unassociated with the identity of that peer. In addition, the outbound traffic and inbound traffic are disassociated from one another; this will make attacks that use traffic analysis much harder to execute.
