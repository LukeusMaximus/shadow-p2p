...
%TODO packets for requests of keysets / position transfer certificates
...
\[Pr(P = p) = \left\{
    ...
    \end{array}
\right.\]
We use this better estimate in the simulation. The results in Figure %TODO result pictures
show that there is now a much smaller proportion of cases which trigger the hard cut-off, as desired. However, this small proportion of cases does leak some information on $V$ as shown.

Fortunately, there is still some information that hasn't been utilised by $B$. So far, a cut-off has always been selected just after the threshold number of members of $V$ havbe been removed from $S$. Instead, we can update our estimate of the gap between address removals as more addresses are removed. Towards the end of the attack where nearly $|V| - t$ addresses have been removed, the estimate will be more accurate and so the hard cut-off is less likely to be triggered. Simulations with this method of estimation obtain the following results: %TODO insert results

Suprisingly, this gives a worse result than before. The members of $V$ are all heavily skewed towards one end of the diagram. The members of $V$ are being removed from $S$ more than other addresses. Therefore it is concluded that the previous method is currently the best known method to use for selecting cut-offs. A better method may exist, however, it should be noted that it already takes a large number %TODO how large a number
of attacks on the same shout group to produce this result so this method should suffice.

\subsection{Number of Attacks Required}

Now the best method from the previous section is analysed to see how many attacks are required on the shout group before the IP addresses of the shout group members can be guessed with some level of confidence.

%TODO appropriate statistical test
