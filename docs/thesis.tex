...
\subsection{Information Required for Complete Network Knowledge}
...
%TODO reliability info; signatures to confirm that a node is where it is in the network
...
%TODO diagram
The packet is divided up into 4 sections. The first section is for routing keys, the second is for the data encryption public key of the receiver, the third is for so-called 'hybrid headers' and the fourth is the data payload field (including data signature).
...
The data payload first has a signature computed using the sending node's private part of their data signature key. This is then appended to the data. The result is then encrypted symmetrically under the same symmetric key generated for the hybrid headers field. This becomes the data payload field.

The final packet is sent to the first node in the random route that was generated.

\subsubsection{Packet Processing En-route}
%TODO here
{\bf Routing Keys:} These keys are used to indicate the direction that the packet should take at any given node. Each node will have a routing key for each dimension and these are known by the sender. The idea behind the routing keys is that when generic data packet is received by a node, the node checks the first routing key within the field and sends the packet in the dimension corresponding to that key. If the key doesn't correspond to one of its routing keys then the data encryption public key (the second field) is checked to see if the data can be decrypted. If it can, decryption of the packet is attempted, otherwise, the packet is dropped.

The routing key is a pair of ($g\prime$, $h\prime$) as described for public key hiding. The node uses the private part of each of their routing keys to see if $f(g\prime, x) = h\prime$. If it does then the packet is to be routed in the direction corresponding to that key. The node removes this pair from the front of the field and adds a random pair of group members to the back. The packet is then sent in the specified direction.
