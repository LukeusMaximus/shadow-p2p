...
%TODO explain shout groups making one node per valid IP
%TODO explain that making shout groups from existing nodes is not so bad
This packet is sent by the node that wishes to create a shout group from existing nodes in the network. The receiver is a node in the network and broadcasts this message to the other nodes. Nodes will compute a proof of work before sending their details to the node making the shout group. The node will select randomly from the responses and then will confirm the groups creation via a third message.
...
This packet is sent from the joining node to a node already in the network and contains details of the joining node's keyset and shout list. The shout list is that of the shout group which the joining node has formed previously. The node in the network can either give the new node one of its virtual positions or it can select a node in the network that should give the node one of its positions. In the first case, the joiner node broadcasts a packet transferring a virtual position to the new node. In the second case, the joining packet is encapsulated within a generic data packet and sent to the node selected to be teh joiner node. This 'passing-the-buck' behaviour is used to improve network balance as much as possible; as a node is better off giving away its virtual positions then the selected joiner node has no reason to refuse this request. Once the selected joiner node receives the joining packet, they give a virtual position through the creation and broadcast of a certificate.

\subsubsection{Join Response}

The node that created the certificate will then inform the new node of its new position in the network. As the node has been in the position being given, the node knows the shout lists needed to contact their downstream neighbours. These shout lists are also sent to the new node. The new node will then need to inform their upstream neighbours of their shout group to complete the connection creation.

\subsubsection{Shout List Change}

This packet contains the shout list of the new node. It is sent to the upstream nodes encapsulated in a generic data packet; the data field is authenticated by the new node. The upstream nodes will verify the signature of the data field and will begin using the shout list as the replacement downstream connection. The connection replaces the previous downstream connection that these nodes had to the joiner node; this old connection would have terminated once the upstream node received the certificate informing them of the change in their downstream neighbour.

\subsection{Packet for transfer of virtual positions}

This packet is broadcast through the network; as such, delivery is in a controlled manner to ensure that unnecessary transmission of the packet does not occur. The diagonal method described in \cite{218454} modified for a uni-directional toroidal network is a good way to achieve this broadcast. The packet contains a certificate from one node giving a virtual position to another.
