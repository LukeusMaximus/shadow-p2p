...
TypeId ShadowClient::GetTypeId (void) {
    static TypeId tid = TypeId ("ShadowClient")
        ...
        .AddAttribute ("Interval", 
                   ...
                   MakeTimeChecker ())
        .AddAttribute ("OwnPort", 
                   "The source port of the outbound packets",
                   UintegerValue (0),
                   MakeUintegerAccessor (&ShadowClient::m_ownPort),
...
void ShadowClient::SetOwnPort(uint16_t port) {
    m_ownPort = port;
}
...
void ShadowClient::StartApplication (void) {
    ...
    if (m_socket == 0) {
        ...
        if (Ipv4Address::IsMatchingType(m_peerAddress) == true) {
            m_socket->Bind(/*InetSocketAddress(m_ownPort)*/);
        } else if (Ipv6Address::IsMatchingType(m_peerAddress) == true) {
            m_socket->Bind(/*Inet6SocketAddress(m_ownPort)*/);
        }
    }
