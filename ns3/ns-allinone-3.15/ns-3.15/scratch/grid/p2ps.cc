...
ShadowClient::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ShadowClient")
    ...
    .AddAttribute ("PacketSize", "Size of echo data in outbound packets",
                   UintegerValue (100),
                   MakeUintegerAccessor (&ShadowClient::SetDataSize,
                                         &ShadowClient::GetDataSize),
                   MakeUintegerChecker<uint32_t> ())
...
void 
ShadowClient::SetRemote (Address ip, uint16_t port)
{
  m_peerAddress = ip;
  m_peerPort = port;
}
...
void 
ShadowClient::SetRemote (Ipv4Address ip, uint16_t port)
{
...
void 
ShadowClient::SetRemote (Ipv6Address ip, uint16_t port)
{
  m_peerAddress = Address (ip);
  m_peerPort = port;
}
...
void 
ShadowClient::StartApplication (void)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void 
ShadowClient::SetDataSize (uint32_t dataSize)
{
...
uint32_t 
ShadowClient::GetDataSize (void) const
{
  NS_LOG_FUNCTION_NOARGS ();
  return m_size;
}
...
void 
ShadowClient::SetFill (std::string fill)
{
...
void 
ShadowClient::SetFill (uint8_t fill, uint32_t dataSize)
{
  if (dataSize != m_dataSize)
    {
      delete [] m_data;
...
void 
ShadowClient::SetFill (uint8_t *fill, uint32_t fillSize, uint32_t dataSize)
{
  if (dataSize != m_dataSize)
    {
      delete [] m_data;
...
void 
ShadowClient::ScheduleTransmit (Time dt)
{
  NS_LOG_FUNCTION_NOARGS ();
...
void
ShadowClient::HandleRead (Ptr<Socket> socket)
{
